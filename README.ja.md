...
呼び出し・演算子の引数式は深さごとの列に配置し、各列の上端を揃えてソース順に並べます。
入力グループは呼び出し枠の右寄せとし、値とは折れ曲がる定義線で接続します。

例示用の図を作るには、グラフ上部の **SVG…** を押し、背景・操作ボタンを含めるか
選んで **SVGを保存** を実行します。画面外のノードも含めて現在の階層全体を
書き出し、折り畳み・言語・配色・グラフ上だけで編集した値を反映します。
文字、ノードごとのグループ、接続線はInkscapeで編集できます。
SVGの編集はL^ソースには反映されません。
