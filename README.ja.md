...
**PageUp / PageDown** で約1画面分ずつ上下にスクロールできます。文字入力中やメニュー操作中は対象外です。

呼び出し・演算子の引数式は深さごとの列に配置し、各列の上端を揃えてソース順に並べます。
入力グループは呼び出し枠の右寄せとし、値とは折れ曲がる定義線で接続します。

再代入・複合代入も `let^` / `var^` と同様に左辺と右辺の対を縦に並べ、対応する要素を
定義線で結びます。見出しは「再代入」「加算再代入」など演算別の名称です。
複合代入8種類とnilチェック付きの`?`形式に対応し、内部展開された複合演算のASTではなく、
ソースに書かれた右辺を表示します。

Call・演算式・添字式・制御構造など、ソースに対応する非リーフノードは、
矢印ボタンまたは右クリックの **折りたたみ・展開を切り替え** から折りたためます。
**Fold All / Unfold All** もこれらのノードを対象とします。折りたたみは表示だけを変更し、
Input/Outputスロットや配置専用グループは個別の折りたたみ対象にしません。

`if^`文は **条件分岐**、`if^`式は **条件選択** と表示します。
条件式は **条件** ボックス内の式グラフとして表示し、条件全体も個別に折りたためます。
パターンマッチ文は **パターン分岐** として各分岐を横並びに、式は **パターン選択** として
各候補を縦並びに表示します。各 **パターン** ボックスは個別に折りたためる式ノードツリーを
含み、`other^`にはパターンボックスを作りません。
`else`のない条件分岐には、条件不成立時に内容の外周を通る実行線を表示します。
各分岐の出口は下部で合流し、実行線は文の追加ボタンまで続きます。
空の文書でも開始点から追加ボタンへ実行線を引きます。
`return^`・`panic^`は実行経路の終端とし、その先への実行線や文の追加ボタンを表示しません。
必ず終端に達する箇所より後ろに既存の記述がある場合は、実行線をつながずに表示を残します。
