...
test('declaration/value ports point left, while expression branches transpose down', async () => {
    ...
    assert.equal(first.children[1].lhat.expressionTree, true);
    assert.equal(first.children[1].children[0].layoutOptions['elk.direction'], 'RIGHT');
    assert(flatten(first.children[1]).some(node => node.lhat?.operandInput));
