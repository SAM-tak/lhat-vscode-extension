...
線は図示のために自動で引き、直接編集する対象にしない。
構造の編集はボックスのドラッグと右クリックメニューから行い、文の追加・挿入には丸い「＋」「＞」から開くテンプレートメニューを使う。
...
### let^ -> 変数定義 (Variable Definition?)

### var^ -> 可変変数定義 (Mutable Variable Definition?)

### def^ -> 型定義 (Definition?)

### enum^ -> 列挙体定義 (Enumeration?)

### errordef^ -> エラー定義 (Error Definition?)
