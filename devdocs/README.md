...
`.lh` を開いた状態で **`L^: Show Graph View`**（日本語UIでは「ビジュアルエディターを表示」、コマンドパレットまたは
エディタ右上のボタン）を実行すると、グラフが開く。
[06-visual-editor.md](06-visual-editor.md) の写像を実装したもので、今のところ
宣言名の入力欄は Enter またはフォーカスを外す操作で LSP リネームを実行し、参照先もまとめて変更する。
確定した名前の長さに合わせ、入力欄と宣言ボックスも拡大・縮小する。
Escape は取り消し。ソースに反映するのは名前変更だけで、一般のシリアライズはまだ行わない。
独立した文字列・数値リテラルは入力欄で編集でき、確定後に長さに合わせて箱をリサイズするが、
...
- 定義の表示名は既定で VS Code の言語に追従する。設定 `lhat.graph.language` を `ja` / `en` にすると、
  グラフ内だけ日本語 / 英語に切り替わる。`auto` または設定を削除すると VS Code に従う。
  変更は開いているグラフにも再起動なしで反映され、日本語 Language Pack は不要。
  初期値を持つ `let^` / `var^` は「変数定義」/「可変変数定義」、AST 上の値なし宣言は
  「変数宣言」/「可変変数宣言」になる。通常の `let^` / `var^` 構文は `=` と初期値が必須で、
  現在の値なし宣言は `def^` / `self^` 内の `abstract^ 名前 : 型` に限る。`string^` / `number^` は「文字列」/
  「数値」の斜体ラベルになる。英語は Variable Definition / Mutable Variable Definition /
  Variable Declaration / Mutable Variable Declaration / Text / Number。
  文字列・数値の入力欄にも型ラベルを表示する。名前・文字列の中身・元ソースは翻訳しない（06 の 3.1）。
  関数・手続き・条件・繰り返し・修飾子・標準メンバ等のハット識別子も自然言語化する。
  色はテキストのシンタックスハイライトと独立し、`workbench.colorCustomizations` の
  `lhat.graph.declaration`、`lhat.graph.type` 等11系統で調整できる。
  関数は水色（`lhat.graph.function`）、手続きは黄色（`lhat.graph.procedure`）で区別する。元の綴りはツールチップに残す。
- 名前の使用箇所をホバーすると、引数や宣言名への細いオレンジ色の参照線を表示する。
  `import^` / `require^` 経由なら対応する読み込み文（別名の定義では右辺の読み込み式）につなぐ。
  言語サーバーの解決結果を使い、ホバーを外すと消える。接続先が非表示・未解決なら線は出さない。
  色は `lhat.graph.referenceLine` で変更できる。`testing/references.lh` で確認できる（06 の 3.2）。
- 初期値を持つ束縛は `[変数定義 y] ← [右辺]`・`[可変変数定義 y] ← [右辺]` の別々の箱で表示する（06 の 5.2）。
  定義線は右辺の左端 → 宣言の右端で、自動描画する
- テーブル・`def^`・`self^{ … }`・`enum^`・`errordef^` のメンバー定義も
  `[名前・型・修飾子] ← [値]` とする。値のない宣言や位置要素は分割しない。
  メンバ行には左右の定義線をつなぎ、関数・手続きの本体には独立した実行線を描く。
...
- ファイルと各手続き・関数の本体には、文字なしの赤い開始ピクトグラムを置き、
  最初の有効な文へ実行線を引く。空のファイルは開始ノードと文を追加する「＋」を表示する
...
- テーブル・`self^`・`def^`・`errordef^`・`enum^` のほか、引数・戻り値・呼び出し引数など
  コンマ区切りリストの要素間に小さな挿入ボタン、末尾に「＋」を表示する。
  クリックすると、そのリストに書ける候補だけの検索付きメニューを開き、通常の WorkspaceEdit として挿入する
- 関数・手続きは引数の型と名前、戻り値型を独立した宣言行に表示し、掘り下げた画面にも残す。
  二項式は左右の値と演算子を別セルにし、演算子の右クリックメニューから置換できる
...
- 右下の半透明の地図（既定140×220px）で全体のどこを見ているかが分かり、掴んで移動できる（06 の 8.4）。
  左辺・上辺のドラッグで幅・高さを個別に、左上隅のドラッグで両方を同時に変更できる。
  折りたたむと半透明の小さな展開ボタンだけが残る。横ずれは離すと基準位置へ滑らかに戻り、縦位置は維持する
- 線は自動描画専用。ホバー時のハンドルや線のドラッグ操作は持たない。
- 文リスト末尾の丸い「＋」、文の直前の丸い「＞」からテンプレートを選び、文を追加・挿入できる。
  右クリックの「文の有効・無効を切り替え」で `#[~ … ]#` を付け外しできる。
  無効な文は囲みの構文を省略し、中身だけを薄く表示する。
